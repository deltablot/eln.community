-- Fix duplicate history entries when approving pending versions
-- Update the trigger to check for a session variable that tells it to skip

DROP TRIGGER IF EXISTS trigger_record_audit ON records;
DROP FUNCTION IF EXISTS record_audit_trigger();

-- Recreate the function to check for skip flag
...
COMMENT ON FUNCTION record_audit_trigger() IS 'Archives record to history only when content changes and skip flag is not set';
